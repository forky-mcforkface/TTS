...
class VitsF0Dataset(F0Dataset):
    ...
    def _compute_and_save_pitch(audio_config, wav_file, pitch_file=None, sample_rate=None):
        ...
        if snd.values.shape[-1] % audio_config.hop_length != 0:
            spec_size = spec_size + 1

        f0 = torch.nn.functional.interpolate(torch.tensor(f0).unsqueeze(0).unsqueeze(0), scale_factor=(spec_size/len(f0),)).squeeze().numpy()[:spec_size]
